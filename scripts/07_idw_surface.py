...
# --- Konfigurasi (LOCKED) ---
# =========================
USE_FIXED_VMAX: bool = True     # kunci skala nilai Z (laju per 100k) untuk normalisasi
FIXED_VMIN, FIXED_VMAX = 0.0, 50.0

# Resolusi grid & padding (meter)
GRID_STEP_M: float = 100.0      # resolusi grid
PAD_M: float = 1000.0           # padding di luar batas wilayah
...
def idw_predict_for_points(
    ...
    for s in range(0, M, chunk_size):
        ...
        for i in range(P.shape[0]):
            ...
            di = d[i, sel]
            if np.any(di == 0):
                out[s + i] = float(sample_values[sel[di == 0]][0])
                continue
...
print(f"   - Rentang nilai (vmin, vmax) laju/100k: ({vmin:.2f}, {vmax:.2f})")

# Normalisasi ke 0–1 (indeks kerawanan)
Zn = (Z - vmin) / (vmax - vmin)
Zn[~np.isfinite(Zn)] = np.nan

# Colormap khusus: putih → merah muda → merah tua
...
val_clipped = np.nan_to_num(np.clip(Zn, 0, 1))
rgb = risk_cmap(val_clipped)[:, :, :3]
rgb_u8 = (rgb * 255).astype(np.uint8)

# filled layer: alpha hampir penuh di area valid
alpha_filled = np.where(mask_grid & np.isfinite(Zn), int(255 * 0.9), 0).astype(
    np.uint8
)
...
# hotspot layer: alpha sesuai indeks kerawanan (non-linear)
val_norm = val_clipped.copy()
...
fg_filled = folium.FeatureGroup(
    name="Permukaan kerawanan (filled)", show=False
).add_to(m)
fg_hotspot = folium.FeatureGroup(
    name="Permukaan kerawanan (hotspots)", show=True
).add_to(m)

ImageOverlay(
    image=str(PNG_FILLED),
    bounds=img_bounds,
    opacity=1.0,
    name="IDW Filled",
).add_to(fg_filled)
...
# Legend: indeks kerawanan 0–1, warna sama dengan peta (putih → merah)
colors = [to_hex(risk_cmap(i / 4)) for i in range(5)]
legend = cm.LinearColormap(colors, vmin=0.0, vmax=1.0)
...
meta = {
    ...
    "bounds_wgs84_padded": {
        "lat_min": float(lat_min_p),
        "lon_min": float(lon_min_p),
        "lat_max": float(lat_max_p),
        "lon_max": float(lon_max_p),
    },
    "idw_params": {"p": P, "k": K, "radius": RADIUS},
    # skala laju asli (per 100k) yang dipakai untuk normalisasi
    "scaling": {"vmin_laju": float(vmin), "vmax_laju": float(vmax)},
    "index_range": {"min": 0.0, "max": 1.0},
    "hotspot_params": {
        "alpha_max": ALPHA_MAX,
        "alpha_gamma": ALPHA_GAMMA,
        "alpha_cutoff_rel": ALPHA_CUTOFF_REL,
    },
    "output_paths": {
